 ...
 */
	...
	//default values
	$ds_id=$_GET['ds_id'];
	$ds_key = htmlspecialchars($_ds->key, ENT_COMPAT, 'UTF-8');
	$ds_name = htmlspecialchars($_ds->name, ENT_COMPAT, 'UTF-8');
	$indexedDoc = htmlspecialchars($_ds->indexedDoc, ENT_COMPAT, 'UTF-8');
	$description=$_ds->description;
	$record = &$_ds->getRecord();
	$recFlds = array();
	foreach($record->fields as $f){
		$recFlds['key'] = $f->key;
		$recFlds['header'] = htmlspecialchars($f->header, ENT_COMPAT, 'UTF-8'); 
	}
	...
	$smartybackend->assign("dss", $dss);
	$smartybackend->assign("dis", $dis);
	$smartybackend->assign("etpls", $etpls);
	$smartybackend->assign("indexedDoc", $indexedDoc);
	$smartybackend->assign("record", $record);
	$smartybackend->assign("recFlds", $recFlds);
	...
	$smartybackend->display('data/edit_hybrid.tpl');
?>