...
//		header('Location: '.$_SERVER['HTTP_REFERER']);
	...
	case 'view_detail':
		foreach((array) @$_COOKIE['vd'] as $key => $val){			if(!array_key_exists($key, (array) @$_POST['vdetail']))
				setcookie('vd['.$key.']', '', time() - 3600, WCHOME.'/objects/');
		}
		foreach($_POST['vdetail'] as $key => $val)
			setcookie('vd['.$key.']', $key, 0, WCHOME.'/objects/');
		setcookie('pagesize', $_POST['pagesize'], 0, WCHOME.'/objects/');
		setcookie('vd[_touched]', '_touched', 0, WCHOME.'/objects/');
		header('Location: '.$_SERVER['HTTP_REFERER']);
	break;
