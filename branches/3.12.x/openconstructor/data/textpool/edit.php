...
<html>
	<head>
		...
		<title><?=$_ds->name?> | <?=$_GET['id'] != 'new' ? htmlspecialchars($_doc['header'], ENT_COMPAT, 'UTF-8').' | '.EDIT_TEXT.($_doc['id'] != $_doc['real_id'] ? DOCUMENT_IS_ALIAS : '') : CREATE_TEXT?></title>
...
<form method="POST" style="margin:0;padding:0" enctype="multipart/form-data" name="f" action="i_textpool.php">
<textarea name="html" style="display:none"><?=$_GET['id'] == 'new' ? '' : htmlspecialchars($_doc['html'], ENT_COMPAT, 'UTF-8')?></textarea>
<input type=hidden name="action" value="<?=$_GET['id']=='new'?'create':'edit'?>_text">
...
<?=F_TEXT_HEADER?>:<input type="button" accesskey="s" onclick="if(!document.all('btn_save').dbtn) sendData()" value="save" style="width:0px;height:0px;"><br>
<input type="text" name="header" value="<?=htmlspecialchars(@$_doc['header'], ENT_COMPAT, 'UTF-8')?>" onpropertychange="if(!this.value.match(new RegExp('[^\\s]','gi'))) disableButton(btn_save,'<?=WCHOME?>/i/default/e/save_.gif'); else disableButton(btn_save,false)" size="64" maxlength="255"><br>
</form>
<hr size="2">
