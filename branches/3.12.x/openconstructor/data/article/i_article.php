 ...
 * 
 * $Id: i_article.php,v 1.11 2007/03/02 10:06:44 sanjar Exp $
 */
...
switch(@$_POST['action'])
{
	case 'create_article':
		...
		$result=$_ds->add($_POST['header'],@$_POST['html'], $date, @$_POST['related']?$_POST['related']:'', $intro);
		if($result)
		{
	...
	case 'edit_article':
		...
		$result=$_ds->update($_POST['id'],$_POST['header'],@$_POST['html'], $date, @$_POST['related']?$_POST['related']:'0', @$_POST['autointro']=='true'?NULL:($intro?$intro:' '));
		if(@$_FILES['image']['size'] && $_FILES['image']['error'] == UPLOAD_ERR_OK)
			$_ds->update_images($_POST['id'], $_FILES['image']['tmp_name']);
		if($result)
	...
	case 'move_documents':
		...
		if($_POST['ds_id'] > 0 && $_POST['dest_ds_id']> 0)
		{
			...
			foreach($ids as $k=>$id)
			{
				...
				$result=$dest_ds->add($_doc['header'],$_doc['content'],$_doc['date'],$_doc['related'],$_doc['intro']);
				if(!$result)
				{
					unset($ids[$k]);
					continue;
...
//				elseif(@file_exists($_SERVER['DOCUMENT_ROOT'].$_ds->imagepath.$_doc['id'].'.'.$_doc['img_type']))
//					$image_intro=$_SERVER['DOCUMENT_ROOT'].$_ds->imagepath.$_doc['id'].'.'.$_doc['img_type'];
				if(!$dest_ds->update_images($result,$image) && $image)
				{
					$failed=true;
					echo 'Article header: '.$_doc['header'][0].'<br>';
				}
			}
