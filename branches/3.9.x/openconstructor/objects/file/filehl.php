...
<br>
<h3><?=EDIT_OBJECT?></h3>
<form name="f" method="POST" action="i_file.php">
	<input type="hidden" name="action" value="edit_filehl">
...
<?php
	...
	foreach($ds as $v)
		echo '<OPTION VALUE="'.$v['id'].'"'.(array_search($v['id'],$ds_ids)!==false?' SELECTED':'').'>'.$v['name'];
?>	
			</select></td>
		</tr>
	</table>
	...
	<table style="margin:5 0" cellspacing="3">
		...
		<tr>
			...
			<td><input type="text" name="listOffset" value="<?=$obj->listOffset?$obj->listOffset:0?>"></td>
		</tr>
		<tr>
			<td nowrap><?=PR_FILES_PER_PAGE?>:</td>
	...
	<table style="margin:5 0" cellspacing="3">
		<tr>
			<td><?=H_ORDER_LIST_BY?></td>
			<td>
				<select name="order">
				<?php
					$order = abs($obj->order);
					foreach($fields as $id => $name) {
						echo '<option value="'.$id.'"'.($id == $order ? ' SELECTED':'').'>'.$name.'</option>';
					}
				?>
				</select>
			</td>
			<td>
				<select name="reverse">
					<option value="1"<?=($obj->order > 0 ? ' SELECTED' : '')?>><?=PR_FILE_SORT_ASC?></option>
					<option value="-1"<?=($obj->order < 0 ? ' SELECTED' : '')?>><?=PR_FILE_SORT_DESC?></option>
				</select>
			</td>
		</tr>
	</table>
