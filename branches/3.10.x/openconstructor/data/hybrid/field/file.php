 ...
 * 
 * $Id: file.php,v 1.7 2007/03/02 10:06:41 sanjar Exp $
 */
	...
	assert(is_object($field) && $field->family == 'file');
...
<form name="f" method="POST" action="../i_hybrid.php" style="padding:0;margin:0;" onsubmit="return prepare();">
	...
	<table border="0" cellspacing="2">
		...
		<tr>
			<td><?=H_DSH_FIELD_MAX_SIZE?>:</td>
			<td><input type="text" name="maxSize" size="20" maxlength="16" value="<?=$field->maxSize?>"></td>
		</tr>
		<tr>
			<td><?=H_DSH_FIELD_FILETYPES?>:</td>
			<td>
			<div class="property">
				<div class="tip"><?=DS_TT_ALLOWED_FILETYPES?></div>
				<input type="text" style="font-family:monospace;" name="file_types" size="56" value="<?=implode(',',$field->types)?>">
			</div>
			</td>
		</tr>
		<tr>
			<td><?=H_DSH_FIELD_BOUNDS?>:</td>
			<td>
			<div class="property">
				<div class="tip"><?=H_DSH_FIELD_IMG_BOUNDS?></div>
				<input type="text" name="imgBounds" size="56" value="<?=$field->getImgBounds()?>">
			</div>
			</td>
		</tr>
		<tr>
