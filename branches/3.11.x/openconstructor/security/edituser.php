...
<title><?=$multiple ? EDIT_USERS_AUTHS : EDIT_USER_AUTHS.' | '.$owner->name?></title>
<link href="../<?=SKIN?>.css" type=text/css rel=stylesheet>
<script language="JavaScript" type="text/JavaScript">
function dsb(){
	if(<?= ($multiple ? true : WCS::decide($owner, 'edit.chmod') || WCS::decide($ownerGrop, 'edituser')) ? 'false' : 'true'?>)
...
<br>
<h3><?=$multiple ? EDIT_USERS_AUTHS : $owner->name." [$owner->login]"?></h3>
<form name="f" method="POST" action="i_security.php">
	<input type="hidden" name="action" value="edit_user">
	<input type="hidden" name="user_id" value="<?=$_GET['id']?>">
	<fieldset style="padding:10" <?=!$owner ? 'disabled' : ''?>><legend><?=H_RES_OWNERS?></legend>
	<table style="margin:5 0" cellspacing="3">
		<tr>
			<td><?=PR_RES_OWNER?></td>
			<td><input type="text" name="owner" size="32" maxlength="32" value="<?=$owner ? $owner->login : ''?>" disabled></td>
		</tr>
		<tr>
			<td><?=PR_RES_GROUP?></td>
			<td><input type="text" name="ownerGroup" size="32" maxlength="32" value="<?=$ownerGroup ? $ownerGroup->name : ''?>" disabled></td>
		</tr>
	</table>
	</fieldset><br>
	...
	<table style="margin:5 0" cellspacing="3" id="auths">
		...
		<?	endforeach;
			...
			<tr>
				<td title='Key: <?=$act?>'><?=$title?></td>
				<td align="middle"><? ctl("oAuths[$act]", $user[0]->sRes->getOwnerBit($act), $user[1]->sRes->getOwnerBit($act))?></td>
				<td align="middle"><? ctl("gAuths[$act]", $user[0]->sRes->getGroupBit($act), $user[1]->sRes->getGroupBit($act))?></td>
			</tr>
		<?	endforeach;
		endif;
