 ...
 * 
 * $Id: datasource.php,v 1.7 2007/03/02 10:06:41 sanjar Exp $
 */
	...
	assert(is_object($field) && $field->family == 'datasource');
	...
	<table border="0" cellspacing="2">
		...
		<tr>
			...
			<td>
				<?php
					$map = $_dsm->getTree();
					// TODO: сделать так чтоб можно было изменять
					$dstype = (array) @$map[$field->type][$_dsm->types[$field->type]];
						if(!array_key_exists($field->fromDS, $dstype)) {
							echo '<span style="color: red;">The datasource was deleted</span>';
						} else {
							echo '<select size="1" name="fromds" DISABLED>';
							echo '<option value='.$field->fromDS.' SELECTED>'.$dstype[$field->fromDS].'</option>';
							echo '</select>';
						}
				?>
			</td>
