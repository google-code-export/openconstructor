 ...
 * 
 * $Id: i_event.php,v 1.10 2007/03/02 10:06:41 sanjar Exp $
 */
...
switch(@$_POST['action'])
{
	case 'create_event':
		...
		$enddate = getTimestamp(@$_POST['end_year'], @$_POST['end_month'], @$_POST['end_day'], @$_POST['end_time']);
		if(@$_POST['autointro']!='true') {
			...
			$intro = null;
		$result = $_ds->add($_POST['header'],@$_POST['html'], $date, $enddate, @$_POST['place'], $intro);
		if($result) {
	...
	case 'edit_event':
		...
		if(substr($intro, 0, 2) == '<P')
			$intro = utf8_substr($intro, utf8_strpos($intro, '>', 1) + 1);
		$intro=str_replace('<P></P>','',$intro);
		$result=$_ds->update($_POST['id'],$_POST['header'],@$_POST['html'], $date, $enddate, @$_POST['place'], @$_POST['autointro'] == 'true' ? null : $intro);
	...
	case 'move_documents':
		...
		if(isset($_POST['ids'])) {
			...
			foreach($ids as $k=>$id)
			{
				...
				$result=$dest_ds->add($_doc['header'],$_doc['content'],$_doc['date'],$_doc['end_date'],$_doc['place'],$_doc['intro']);
				if(!$result)
				{
					unset($ids[$k]);
					continue;
				...
				{
					$failed=true;
					echo 'Event header: '.$_doc['header'].'<br>';
				}
			}
