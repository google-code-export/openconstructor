 ...
 * 
 * $Id: eventcalendar.php,v 1.9 2007/03/02 10:06:41 sanjar Exp $
 */
...
<br>
<h3><?=EDIT_OBJECT?></h3>
<form name="f" method="POST" action="i_event.php">
	<input type="hidden" name="action" value="edit_eventcalendar">
...
?>	
	...
	<table style="margin:5 0" cellspacing="3">
		<tr>
			<td nowrap><?=PR_CALENDAR_HEADER?>:</td>
			<td><input type="text" name="header" value="<?=htmlspecialchars(@$obj->header, ENT_COMPAT, 'UTF-8')?>"></td>
		</tr>
		<tr>
			<td nowrap><?=PR_CALENDAR_YEAR?>:</td>
			<td><input type="text" name="year" value="<?=intval(@$obj->year)?>"></td>
		</tr>
		<tr>
			<td nowrap valign=top><?=PR_MONTH_NAMES?>:</td>
			<td><textarea cols="41" rows="5" name="month"><?=htmlspecialchars(implode(',',@$obj->months), ENT_COMPAT, 'UTF-8')?></textarea></td>
		</tr>
		<tr>
		...
		<tr>
			<td nowrap><?=PR_EVENT_URI?>:</td>
			<td><input type="text" name="srvuri" value="<?=$obj->srvuri?>"></td>
		</tr>
		<tr>
			<td nowrap><?=PR_EVENT_ID?>:</td>
			<td><input type="text" name="eventid" value="<?=$obj->eventid?>"></td>
		</tr>
		<tr>
			<td nowrap><?=PR_MONTH_ID?>:</td>
			<td><input type="text" name="monthid" value="<?=$obj->monthid?>"></td>
		</tr>
		<tr>
			<td nowrap><?=PR_MO_WEEK?></td>
			<td><select name="moWeek">
				<option value="true" <?=@$obj->moWeek?'selected':''?>><?=PR_MO_WEEK_MO?></option>
				<option value="false" <?=!@$obj->moWeek?'selected':''?>><?=PR_MO_WEEK_SU?></option>
			</select></td>
		</tr>
	</table>
	</fieldset><br>
