 ...
 * 
 * $Id: galleryimage.php,v 1.8 2007/03/02 10:06:41 sanjar Exp $
 */
...
function setdynamic(stat){
	f.all('ds_id[]').disabled=!(f.isPub.disabled=f.dyn_ds_id.disabled=!stat);
}
</script>
</head>
...
<br>
<h3><?=EDIT_OBJECT?></h3>
<form name="f" method="POST" action="i_gallery.php">
	<input type="hidden" name="action" value="edit_galleryimage">
...
?>
	<fieldset style="padding:10"><legend><?=PR_DATASOURCES?></legend>
	<table style="margin:5 0" cellspacing="3">
		<tr>
			<td nowrap valign="top"><input type=radio checked name=dynamic_ds value="false" onclick="setdynamic(!this.checked)"> <?=PR_STATIC_GALLERIES?>:</td>
			<td><select size="10" name="ds_id[]" multiple  <?=$obj->dynamic_ds?'disabled':''?>>
<?php
...
?>	
		...
		<tr>
			<td nowrap valign="top"><input type=radio <?=@$obj->dynamic_ds?'checked':''?> name=dynamic_ds value="true" onclick="setdynamic(this.checked)"> <?=PR_PHOTO_PUBLICATION_ID?>:</td>
			<td><input type="text" name="dyn_ds_id" <?=@$obj->dynamic_ds?'':'disabled'?> value="<?=@$obj->dynamic_ds?$obj->ds_id:'publication_id'?>"></td>
		</tr>
		<tr>
			<td nowrap valign="top" colspan=2><input type=checkbox name=isPub value="true" <?=@$obj->isPub?'checked':''?> <?=@$obj->dynamic_ds?'':'disabled'?>> <?=PR_IS_PUBLICATION_ID?></td>
		</tr>
	</table>
	</fieldset><br>
	...
	<table style="margin:5 0" cellspacing="3">
		...
		<tr>
			<td nowrap><?=PR_IMAGE_ID?></td>
			<td><input type="text" name="imageid" value="<?=$obj->imageid?>"></td>
		</tr>
		<tr>
			<td colspan="2"><input type="checkbox" name="byId" value="true" <?=$obj->byId ? 'checked': ''?>> <?=PR_GET_IMAGE_BY_ID?></td>
		</tr>
		<tr>
