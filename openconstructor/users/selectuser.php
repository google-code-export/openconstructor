...
<title><?=H_SELECT_USERS?></title>
	...
	<script language="JavaScript" type="text/JavaScript">
		...
		function startsearch() {
			...
			results.src = "users.php?type=" + type + "&keyword=" + query;
		}
		...
		function selectUsers() {
			var select = null;
			try {
				select = document.getElementById("f.result").contentWindow.document.getElementById("s.users");
			} catch(e) {return false;}
			if(select.multiple) {
				for(var i = 0; i<select.options.length; i++)
					if(select.options[i].selected)
						returnValue[returnValue.length] = [select.options[i].value, select.options[i].getAttribute("login"), select.options[i].innerHTML];
			} else
...
<table width="100%" cellpadding="0" cellspacing="0" border="0" height="100%">
	...
	<tr><td><iframe src="users.php" height="100%" width="100%" id="f.result" frameborder="0"></iframe></td></tr>
	<tr height="40">
		<td align="right" style="padding:0 5px;border-top:solid 1px #999;">
			<input type="button" value="<?=BTN_SELECT?>" id="btn.select" disabled onclick="selectUsers();"> <input type="button" value="<?=BTN_CANCEL?>" onclick="window.close();">
		</td>
	</tr>
</table>
</body>
