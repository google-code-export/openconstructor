...
<html>
	<head>
		...
		<title><?=$_ds->name?> | <?=$_GET['id']=='new'?CREATE_EVENT:htmlspecialchars($_doc['header'], ENT_COMPAT, 'UTF-8').' | '.EDIT_EVENT.($_doc['id']!=$_doc['real_id']?DOCUMENT_IS_ALIAS:'')?></title>
...
<script>
	...
	function sendData()
	{
		...
		f.place.value=eventplace.document.body.innerHTML;
		...
		eventplace.document.body.disabled=true;
		eventplace.document.designMode="Off";
		disableButton(btn_save,'<?=WCHOME?>/i/e/save_.gif');
...
<form method="POST" style="margin:0;padding:0;" enctype="multipart/form-data" name="f" action="i_event.php">
...
<textarea name="place" style="display:none"><?=$_GET['id']=='new'?'':htmlspecialchars($_doc['place'], ENT_COMPAT, 'UTF-8')?></textarea>
<input type=hidden name="action" value="<?=$_GET['id']=='new'?'create':'edit'?>_event">
...
<?=F_EVENT_HEADER?>:<input type="button" accesskey="s" onclick="if(!document.all('btn_save').dbtn) sendData()" value="save" style="width:0px;height:0px;"><br>
...
<tr><td nowrap style="padding-right:15px" valign="top"><?=F_DATE_TIME_BEGIN?>:<br>
<?php
...
<td nowrap colspan=2 style="padding-right:0px" valign="top"><?=F_DATE_TIME_END?>:<br>
<?php
	list($day, $month, $year) = explode('/', date('d/m/Y', $_doc['end_date']));
	echo '<input type="text" name="end_day" size="3" maxlength="2" value="'.$day.'">';
	echo '<select size=1 name="end_month">';
	for($i=1;$i<=12;$i++)
	{
		echo '<option value='.$i;
		if($i==$month) echo ' selected';
	...
	echo '</select>';
	echo '<select size=1 name="end_year">';
	for($i=1999;$i<=2010;$i++)
	{
		echo '<option value='.$i;
		if($i==$year) echo ' selected';
	...
	echo '</select>';
?>&nbsp;<input type="text" name="end_time" size="8" maxlength="8" value="<?=date('H:i:s', $_doc['end_date'])?>">
<input type=checkbox <?=@$_doc['published']?' checked':''?> <?=!WCS::decide($_ds, 'publishdoc')||$_GET['id']=='new'?'disabled':''?> name="published" value="true"> <?=F_IS_PUBLISHED?></td>
</tr>
<?php if($_GET['id']!='new') {
		if(@$_ds->images['main']||$_ds->images['intro']) echo '<tr><td colspan=3><hr style="margin:0px;padding:0px;"></td></tr>';
?>
<tr><td nowrap valign="top">
<?php if(@$_ds->images['main']) {
...
</td></tr><tr height="75%" valign="top"><td colspan=2>
...
</td><td style="padding:2px 0px">
<nobr><?=F_EVENT_PLACE?>: </nobr>
</td></tr><tr height="25%" valign="top"><td width="50%" style="padding: 0px 10px 0px 0px;">
<iframe name="introcontent" style="border:none" width="100%" style="margin:0" height="100%" onfocus="theHTML=introcontent"></iframe>
</td><td width="50%">
<iframe name="eventplace" style="border:none" width="100%" style="margin:0" height="100%" onfocus="theHTML=eventplace"></iframe>
</td></tr><table>
<script defer>
	theHTML=content;
	content.document.designMode="On";
	introcontent.document.designMode="On";
	eventplace.document.designMode="On";
	...
	window.inter2=window.setInterval(setcontent2,500);
	function setcontent2(a)
	{
		if(eventplace.document.body)
		{
			window.clearInterval(window.inter2);
			eventplace.document.body.innerHTML=f.place.value;
			eventplace.document.body.onkeypress=new Function('',"if(window.eventplace.event.keyCode==28) {window.parent.editsource()}");
			head=eventplace.document.childNodes(0).childNodes(0);
			link=eventplace.document.createElement('<link href="http://<?=$_host?>/css/content.css" type=text/css rel=stylesheet>');
			head.appendChild(link);
			eventplace.document.body.style.background='white';
			eventplace.document.body.style.margin='5px';
		}
	}
</script>
</body>
